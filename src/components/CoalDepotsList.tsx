import { Cloudinary } from "@cloudinary/url-gen";
import { AdvancedImage, lazyload, responsive } from "@cloudinary/react";
import Link from "next/link";

interface CoalDepot {
  id: string;
  latitude: number;
  longitude: number;
  address: string;
  publicId: string;
  coalDepotName: string;
}

interface IProps {
  coalDepots: CoalDepot[];
  setHighligtedId: (id: string | null) => void;
  loading: boolean;
}

const CoalDepotsList = ({ coalDepots, setHighligtedId, loading }: IProps) => {
  const cld = new Cloudinary({
    cloud: {
      cloudName: process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME,
    },
  });

  if (loading)
    return (
      <div>
        <CoalDepotListSkeleton />
        <CoalDepotListSkeleton />
        <CoalDepotListSkeleton />
        <CoalDepotListSkeleton />
      </div>
    );

  return (
    <>
      {coalDepots.map((coalDepot) => (
        <Link href={`/coal-depots/${coalDepot.id}`} key={coalDepot.id}>
          <div
            className="mx-1 md:mx-2 mt-2 cursor-pointer flex flex-col"
            onMouseEnter={() => setHighligtedId(coalDepot.id)}
            onMouseLeave={() => setHighligtedId(null)}
          >
            <div className="bg-nav flex flex-col md:flex-row p-1 md:p-4 rounded-lg ">
              <div className="sm:w-full md:w-1/2  ">
                <AdvancedImage
                  cldImg={cld.image(coalDepot.publicId)}
                  plugins={[lazyload(), responsive()]}
                  className="rounded-lg w-80 h-56 object-cover"
                />
              </div>
              <div className="sm:w-full md:w-1/2 sm:pl-0 md:pl-4 flex flex-col justify-center p-2 text-center md:text-left ">
                <h2 className="text-sm md:text-xl font-bold ">
                  {coalDepot.address}
                </h2>
                <h3 className="text-sm md:text-lg font-semibold">
                  {coalDepot.coalDepotName}
                </h3>
              </div>
            </div>
          </div>
        </Link>
      ))}
    </>
  );
};

export default CoalDepotsList;

const CoalDepotListSkeleton = () => {
  return (
    <div className="mx-1 md:mx-2 mt-2 cursor-pointer flex flex-col">
      <div className="bg-nav flex flex-col md:flex-row p-1 md:p-4 rounded-lg ">
        <div className="sm:w-full md:w-1/2 rounded-lg  ">
          <div className="bg-skeleton w-80 h-56 flex justify-center items-center">
            <svg
              className="w-12 h-12 text-nav"
              xmlns="http://www.w3.org/2000/svg"
              aria-hidden="true"
              fill="currentColor"
              viewBox="0 0 640 512"
            >
              <path d="M480 80C480 35.82 515.8 0 560 0C604.2 0 640 35.82 640 80C640 124.2 604.2 160 560 160C515.8 160 480 124.2 480 80zM0 456.1C0 445.6 2.964 435.3 8.551 426.4L225.3 81.01C231.9 70.42 243.5 64 256 64C268.5 64 280.1 70.42 286.8 81.01L412.7 281.7L460.9 202.7C464.1 196.1 472.2 192 480 192C487.8 192 495 196.1 499.1 202.7L631.1 419.1C636.9 428.6 640 439.7 640 450.9C640 484.6 612.6 512 578.9 512H55.91C25.03 512 .0006 486.1 .0006 456.1L0 456.1z" />
            </svg>
          </div>
        </div>
        <div className="sm:w-full md:w-1/2 sm:pl-0 md:pl-4 flex flex-col justify-center p-2 text-center md:text-left ">
          <div className="h-2 bg-skeleton rounded-full  max-w-[360px] mb-2.5"></div>

          <div className="h-2 bg-skeleton rounded-full  max-w-[400px]"></div>
        </div>
      </div>
    </div>
  );
};
